...
import transformers
from transformers import BertPreTrainedModel, BertForSequenceClassification, BertModel, BertLMHeadModel
from transformers import RobertaForSequenceClassification, RobertaModel, RobertaForMaskedLM
...
def resize_token_type_embeddings(model, new_num_types: int, random_segment: bool):
    ...
    if not random_segment:
        new_token_type_embeddings.weight.data[:old_token_type_embeddings.weight.size(
            0)] = old_token_type_embeddings.weight.data
...
class BertForPromptFinetuning(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls = BertLMHeadModel(config)
        self.init_weights()
    ...
    def forward(
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                labels = torch.stack([1 - (labels.view(-1) - self.lb) / (
                    self.ub - self.lb), (labels.view(-1) - self.lb) / (self.ub - self.lb)], -1)
...
class RobertaForPromptFinetuning(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classification_roberta = RobertaForSequenceClassification(config)
        self.classifier = self.classification_roberta.classifier
    ...
    def forward(
        ...
        if self.return_full_softmax:
            if labels is not None:
                return torch.zeros(1, out=outputs.logits.new()), outputs.logits
            return outputs.logits

        # Return logits for each label
        logits = []
        for label_id in range(len(self.label_word_list)):
            logits_label = outputs.logits[[i for i in range(
                len(mask_pos))], mask_pos, self.label_word_list[label_id]].unsqueeze(-1)
            logits.append(logits_label)
        logits = torch.cat(logits, -1)
