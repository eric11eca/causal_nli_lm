...
def tokenize_multipart_input(
    ...
    if prompt:
        ...
        *xx* represent variables:
        ...
        # Current segment id. Segment id +1 if encountering sep+.
        segment_id = 0
        ...
        for part_id, part in enumerate(template_list):
            ...
            if part in special_token_mapping:
                ...
                # if len(part) == 1:
                #    new_tokens += tokenizer.encode(part)
                # else:
                new_tokens += enc(part)
    ...
    if first_sent_limit is not None and len(input_ids) > max_length:
        ...
        logger.warn("Input exceeds max_length limit: {}".format(
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        if args.prompt:
            ...
            for key in self.label_to_word:
                ...
                if self.label_to_word[key][0] not in ['<', '[', '.', ',']:
                    ...
                    assert len(tokenizer.tokenize(
                        ' ' + self.label_to_word[key])) == 1
                    self.label_to_word[key] = tokenizer.encode(
                        ' ' + self.label_to_word[key], add_special_tokens=False)
                else:
                    self.label_to_word[key] = tokenizer.encode(
                        self.label_to_word[key], add_special_tokens=False)

            if len(self.label_list) > 1:
                self.label_word_list = [self.label_to_word[label]
        ...
        if args.prompt and len(args.template_list) > 0:
        ...
        logger.info("Total num_sample for mode %s: %d" %
                    (mode, self.num_sample))
        ...
        logger.info(
            f"Creating/loading examples from dataset file at {args.data_dir}")
        ...
        if self.use_demo and args.demo_filter:
            ...
            self.support_emb = np.load(os.path.join(
                args.data_dir, "train_{}.npy".format(args.demo_filter_model)))
            self.query_emb = np.load(os.path.join(
                args.data_dir, "{}_{}.npy".format(split_name, args.demo_filter_model)))
        ...
        print("NUmber of examples: ", self.num_sample)
        # Size is expanded by num_sample
        self.size = len(self.query_examples) * self.num_sample
        print("Number of training data: ", self.size)
            ...
            for query_idx in range(len(self.query_examples)):
                ...
                if self.use_demo and args.demo_filter:
                    ...
                    for support_idx in candidate:
                        sim_score.append((support_idx, util.pytorch_cos_sim(
                            self.support_emb[support_idx], self.query_emb[query_idx])))
                    ...
                    if self.num_labels == 1:
                        ...
                        for support_idx, score in sim_score:
                            if count_each_label['0' if float(self.support_examples[support_idx].label) <= median_mapping[args.task_name] else '1'] < limit_each_label:
                                count_each_label['0' if float(
                                    self.support_examples[support_idx].label) <= median_mapping[args.task_name] else '1'] += 1
                                context_indices.append(support_idx)
                                if args.debug_mode:
                                    print("    %.4f %s | %s" % (
                                        score, self.support_examples[support_idx].label, self.support_examples[support_idx].text_a))  # debug
                    else:
                        limit_each_label = int(
                            len(sim_score) // self.num_labels * args.demo_filter_rate)
                        count_each_label = {
                            label: 0 for label in self.label_list}
                        context_indices = []
        ...
        if mode != "train":
            ...
            for query_idx, context_indices, bootstrap_idx in self.example_idx:
                ...
                if len(args.template_list) > 0:
                    template = args.template_list[sample_idx % len(
                        args.template_list)]  # Use template in order
                else:
        ...
        if self.args.gpt3_in_context_head or self.args.gpt3_in_context_tail:
            ...
            for i in order:
                ...
                if len(self.label_list) == 1:
                    # Regression
                    label = '0' if float(
                        label) <= median_mapping[self.args.task_name] else '1'
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            if len(self.args.template_list) > 0:
                template = self.args.template_list[sample_idx % len(
                    self.args.template_list)]
            else:
    ...
    def convert_fn(
        ...
        if not use_demo:
            ...
            query_text = input_example_to_tuple(
                example)  # Input sentence list for query
            ...
            if self.args.gpt3_in_context_head or self.args.gpt3_in_context_tail:
                ...
                for support_example in supports:
                    augmented_example += input_example_to_tuple(
                        support_example)
                    current_label = support_example.label
                    if len(label_list) == 1:
                        current_label = '0' if float(
                            current_label) <= median_mapping[self.args.task_name] else '1'  # Regression
        ...
        if verbose:
            ...
            logger.info(
                f"text: {self.tokenizer.decode(features.input_ids, skip_special_tokens=True)}")
