...
def main():
    ...
    if data_args.prompt:
        if data_args.prompt_path is not None:
            ...
            logger.info("Specify load the %d-th prompt: %s | %s" %
            ...
            if data_args.template_path is not None:
                ...
                logger.info("Load top-%d templates from %s" %
    ...
    try:
        ...
        logger.info("Task name: {}, number of labels: {}, output mode: {}".format(
            ...
            for instance_id in range(data_args.gpt3_in_context_num):
                ...
                for sent_id in range(sent_num):
                    sub_template = sub_template.replace(
                        "_{}*".format(sent_id), "_{}*".format(sent_num + sent_num * instance_id + sent_id))
                # Replace mask
                sub_template = sub_template.replace(
            ...
            if data_args.template_list is not None:
                for i in range(len(data_args.template_list)):
                    ...
                    for label_id in range(num_labels):
                        ...
                        for sent_id in range(sent_num):
                            sub_template = sub_template.replace(
                                "_{}*".format(sent_id), "_{}*".format(sent_num + sent_num * label_id + sent_id))
                        # Replace mask
                        sub_template = sub_template.replace(
                ...
                for label_id in range(num_labels):
                    ...
                    # Replace sent id
                    for sent_id in range(sent_num):
                        sub_template = sub_template.replace("_{}".format(
                            sent_id), "_{}".format(sent_num + sent_num * label_id + sent_id))
                    # Replace mask
                    sub_template = sub_template.replace(
                        "*mask*", "*label_{}*".format(label_id))
                    new_template = new_template + sub_template
    ...
    class ContextualizedEncoderPromptModel(pl.LightningModule):
        def __init__(self, model, tokenizer, data_args, model_args, training_args):
            super().__init__()
    ...
    if training_args.do_train:
        trainer.train(model_path=model_args.model_name_or_path if os.path.isdir(
            ...
            if trainer.is_world_master():
                with open(output_eval_file, "w") as writer:
                    ...
                    for key, value in eval_result.items():
                        ...
                        writer.write("%s = %s\n" % (key, value))
                        final_result[eval_dataset.args.task_name +
                                     '_dev_' + key] = value
    ...
    if training_args.do_predict:
        ...
        if data_args.task_name == "mnli":
            ...
            test_datasets.append(
                FewShotDataset(mnli_mm_data_args, tokenizer=tokenizer, mode="test", use_demo=(
                    'demo' in model_args.few_shot_type))
            )
            ...
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    ...
                    for key, value in test_result.items():
                        logger.info("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
                        final_result[test_dataset.args.task_name +
                                     '_test_' + key] = value
                ...
                if training_args.save_logit:
                    ...
                    np.save(os.path.join(training_args.save_logit_dir, "{}-{}-{}.npy".format(
                        test_dataset.task_name, training_args.model_id, training_args.array_id)), logits)
